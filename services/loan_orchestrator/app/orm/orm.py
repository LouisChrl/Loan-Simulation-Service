...
class Loan(Base):
    ...
    loan_amount: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    loan_status: Mapped[str] = mapped_column(String(100), nullable=False)

class LoanMonitoring(Base):
    __tablename__ = "loan_monitoring"
    id: Mapped[int] = mapped_column(primary_key=True)
    loan_id: Mapped[int] = mapped_column(ForeignKey("loan.id"), nullable=False)
    risk_status: Mapped[str] = mapped_column(String(100), nullable=False)
    loan_provider_status: Mapped[str] = mapped_column(String(100), nullable=False)